class Assert {
    ...
    IsNull(obj, msg) {
        if (obj != null) {
            this.Fail(msg || '理应为 Null (Should be Null)');
            return;
        }
        this.Pass();
    }
    ...
    IsNotNull(obj, msg) {
        if (obj == null) {
            this.Fail(msg || '理应不为 Null (Should not be Null)');
            return;
        }
        this.Pass();
    }
    ...
    IsTrue(bool, msg) {
        if (!bool) {
            this.Fail(msg || '理应为 True 但得到了 False (Should be True)');
            return;
        }
        this.Pass();
    }
    ...
    IsFalse(bool, msg) {
        if (bool) {
            this.Fail(msg || '理应为 False 但得到了 True (Should be False)');
            return;
        }
        this.Pass();
    }
    ...
    AreEqual(value, should, msg) {
        if (value && typeof value.Equals == 'function') {
            if (!value.Equals(should)) {
                this.Fail(msg || `理应相等(Should be equal)\r\n${value.toString()}\r\n${should.toString()}`);
                return;
            }
    ...
    AreNotEqual(value, notShould, msg) {
        if (value && typeof value.Equals == 'function') {
            if (value.Equals(notShould)) {
                this.Fail(msg || `理应不相等(Should not be equal)\r\n${value.toString()}\r\n${notShould.toString()}`);
                return;
            }
    ...
    Result() {
        console.log(`\r\n模块(Module)"${this._moduleName}"
测试总数(Totle count)：${this._passCount + this._errorCount}
通过测试数(Success count)：${this._passCount}
错误测试数(Error count)：${this._errorCount}
${this._errorCount ? "错误列表(Error list)：\r\n%c" + this._errorList.join("\r\n\r\n") : "%c"}\r\n`, 'color:red');
    }
